...
let state = {
    ...
    grayscale: false, fontFamily: 'Anton', textColor: '#ffffff', format: 'original'
};
...
function requestDraw() {
    if (isDrawing) { drawingScheduled = true; return; }
...
function drawCanvas() {
    ...
    // 1. Optimisation Taille (Max 1200px)
    const MAX_SIZE = 1200;
    let scale = 1;
    ...
    if (state.format === 'square') {
        ...
        dx = (side - trueWidth) / 2; dy = (side - trueHeight) / 2;
    } else if (state.format === 'story') {
        const targetRatio = 9 / 16;
        if ((trueWidth / trueHeight) > targetRatio) {
            canvas.height = trueHeight; canvas.width = trueHeight * targetRatio;
        } else {
            canvas.width = trueWidth; canvas.height = trueWidth / targetRatio;
        }
        dWidth = trueWidth; dHeight = trueHeight;
        dx = (canvas.width - trueWidth) / 2; dy = (canvas.height - trueHeight) / 2;
    } else {
        canvas.width = trueWidth; canvas.height = trueHeight;
        dWidth = trueWidth; dHeight = trueHeight; dx = 0; dy = 0;
    }
    ...
    try {
        ...
        // Conversion en décimales pour iOS (ex: 1.5 au lieu de 150%)
        ...
        if (state.grayscale) filterString += ' grayscale(1)';
        ...
        isDrawing = false;
        if (drawingScheduled) { drawingScheduled = false; requestDraw(); }
    }
}
...
imageUpload.addEventListener('change', (e) => {
    ...
    if (file) {
        ...
        reader.onload = (event) => {
            const img = new Image();
            img.onload = () => { state.image = img; requestDraw(); };
...
presetButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
        const s = presets[e.target.dataset.preset];
        state = { ...state, ...s };
        brightnessSlider.value = s.brightness; contrastSlider.value = s.contrast; grayscaleToggle.checked = s.grayscale;
        requestDraw();
    });
});
formatButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
        formatButtons.forEach(b => b.classList.remove('active')); e.currentTarget.classList.add('active');
        state.format = e.currentTarget.dataset.format; requestDraw();
    });
});
resetBtn.addEventListener('click', () => {
    state = { ...state, topText:'', bottomText:'', brightness:100, contrast:100, saturation:100, sepia:0, grayscale:false, format:'original' };
    topTextInput.value=''; bottomTextInput.value=''; brightnessSlider.value=100; contrastSlider.value=100; grayscaleToggle.checked=false;
    formatButtons.forEach(b => b.classList.remove('active')); document.querySelector('[data-format="original"]').classList.add('active');
    requestDraw();
});
